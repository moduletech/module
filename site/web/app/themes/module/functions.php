...
//
// Pull OG and Meta Tags from a URL
//
function get_meta_by_url($url) {
  ...
  foreach($html->getElementsByTagName('meta') as $meta) {
    if($meta->getAttribute('property')) {
      $meta_tags[$meta->getAttribute('property')] = $meta->getAttribute('content');
    } else {
      $meta_tags[$meta->getAttribute('name')] = $meta->getAttribute('content');
    }
  }
  return $meta_tags;
}
...
//
// Scrape Article for local/remote Meta
//
function article_meta($postid) {
  // Compile Article Information
  $article = array();
  $article['id'] = $postid;
  $article['url'] = get_post_meta($article['id'], 'article_url', true);

  // Get Locally Specified Post Info
  $article['title'] = get_post_meta($article['id'], 'custom_title', true);
  $article['description'] = get_post_meta($article['id'], 'custom_description', true);

  // For blank values, check locally
  foreach($article as $name=>$value) {
    if(!$value) {
      $local_meta = get_post_meta($article['id'],'sourced_'.$name,true);
      if($local_meta) {
        $article[$name] = $local_meta;
      } else {
        $article['needs_remote'] = true;
      }
    }
  }

  // For unchecked values, check remotely
  if(array_key_exists('needs_remote',$article) && $article['needs_remote']) {
    $remote_meta = get_meta_by_url($article['url']);
    foreach($article as $name=>$value) {
      if(!$value) {
        if(array_key_exists($name,$remote_meta)) {
          $article[$name] = $remote_meta[$name];
        } elseif (array_key_exists('og:'.$name,$remote_meta)) {
          $article[$name] = $remote_meta['og:'.$name];
        } elseif (array_key_exists('twitter:'.$name,$remote_meta)) {
          $article[$name] = $remote_meta['twitter:'.$name];
        }
        if($article[$name]) {
          update_post_meta($article['id'],'sourced_'.$name,$article[$name]);
        }
      }
    }
  }
