<footer class="footer" itemscope itemtype="http://schema.org/WPFooter">
    <div class="footer-logo__wrapper">
        <img class="footer-logo" src="<?php echo get_stylesheet_directory_uri();?>/assets/img-dist/logo-white.svg" alt="Module Logo">
    </div>

    <div class="footer-partners u-text-center">
        <div class="u-col-four">
            <a href="http://techstars.com" target="_blank">
                <img class="footer-partners__logo" src="<?php echo get_stylesheet_directory_uri();?>/assets/img-dist/partners/techstars.png" alt="TechStars">
            </a>
        </div>
        <div class="u-col-four">
            <a href="http://alphalabgear.org" target="_blank">
                <img class="footer-partners__logo" src="<?php echo get_stylesheet_directory_uri();?>/assets/img-dist/partners/alphalabgear.png" alt="Alphalab Gear">
            </a>
        </div>
        <div class="u-col-four">
            <a href="http://ideafoundry.org" target="_blank">
                <img class="footer-partners__logo" src="<?php echo get_stylesheet_directory_uri();?>/assets/img-dist/partners/ideafoundry.png" alt="Idea Foundry">
            </a>
        </div>
    </div>

    <div class="footer-links u-text-center">
        <a class="footer-link u-link-white" href="<?php echo bloginfo('url')?>/home">Home</a>
        <a class="footer-link u-link-white" href="<?php echo bloginfo('url')?>/reservations">Reservations</a>
        <a class="footer-link u-link-white" href="<?php echo bloginfo('url')?>/jobs">Jobs</a>
        <a class="footer-link u-link-white" href="<?php echo bloginfo('url')?>/press">Press</a>
        <a class="footer-link u-link-white" href="<?php echo bloginfo('url')?>/contact">Contact Us</a>
    </div>
    <p class="copyright u-text-white u-text-center">
        Copyright &copy; <?php echo Date('Y');?> Module Design, Inc.
    </p>
</footer>